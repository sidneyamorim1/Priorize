...
export const TaskModal: React.FC<TaskModalProps> = ({
  ...
  useEffect(() => {
    if (taskToEdit) {
      ...
      setAutoStart(taskToEdit.autoStart || false);
    } else {
      setTitle('');
      setDescription('');
      setPriority('medium');
      ...
      // Define a data de início como o dia de hoje e o prazo final para 30 dias depois
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg transform rounded-2xl border border-slate-100 bg-white p-6 shadow-2xl transition-all duration-300 animate-scale-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Grid Datas (Início e Fim) */}
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {/* Data de Início */}
            <div>
              <label htmlFor="modal-start-date" className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1.5">
          ...
          {/* Começar Automaticamente Toggle */}
          <div className="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50/50 p-3.5 transition-all">
            <div className="flex flex-col gap-0.5">
              <span className="text-sm font-semibold text-slate-700">Começar Automaticamente</span>
              <span className="text-xs text-slate-400">A tarefa iniciará (moverá para 'Fazendo') na data programada.</span>
            </div>
            <button
              type="button"
              ...
              className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-brand-500/20 ${
                autoStart ? 'bg-brand-500' : 'bg-slate-200'
              }`}
            >
              <span
